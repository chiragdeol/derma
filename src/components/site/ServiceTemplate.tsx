...
const FAQItemComponent = ({ question, answer }: FAQItem) => {
  ...
  return (
    <div className="border-b border-border/60">
      ...
      >
        <span>{question}</span>
        <span className={`text-[#7B8D6A] text-xl transition-transform duration-300 ${open ? 'rotate-45' : ''}`}>
          +
        </span>
      </button>
    ...
    <section className="py-20 bg-card border-y border-border/60">
      <div className="mx-auto max-w-6xl px-6 lg:px-10 grid gap-12 lg:grid-cols-[1.1fr_0.9fr] items-center">
        ...
        >
          ...
          <div 
            style={{ left: `${position}%` }}
            className="absolute top-0 bottom-0 w-0.5 bg-[#7B8D6A] -translate-x-1/2 pointer-events-none"
          >
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-card border border-[#7B8D6A] shadow-md flex items-center justify-center text-[#7B8D6A] font-bold text-xs select-none">
              ⇆
            </div>
          </div>
        </div>
              ...
              <button
                ...
                className={`text-xs px-4 py-2 rounded-full border transition-all duration-300 font-sans cursor-pointer ${
                  activeTreatment === t.name
                    ? 'border-[#7B8D6A] text-primary bg-background font-medium'
                    : 'border-border/80 text-muted-foreground hover:border-[#7B8D6A]/60'
                }`}
              >
                {t.name}
              </button>
    ...
    <>
      ...
      <section className="py-12 md:py-16">
        <div className="mx-auto max-w-6xl px-6 lg:px-10 grid gap-12 lg:grid-cols-[1.1fr_0.9fr] items-center">
          <div>
            ...
            <h1 className="font-display text-4xl md:text-5xl lg:text-6xl text-foreground mb-6 leading-tight">
              {h1.includes("Sharjah") ? (
                <>
                  {h1.replace("Sharjah", "")}
                  <em className="italic text-[#7B8D6A] font-light">Sharjah</em>
                </>
              ) : (
                h1
              )}
            ...
            <div className="flex gap-4 flex-wrap mb-8">
              <a href="https://wa.me/971500999324" className="rounded-lg bg-[#7B8D6A] text-black px-6 py-3.5 text-sm font-semibold hover:opacity-95 transition-all shadow-sm">
                Book a consultation
              </a>
              <a href="https://wa.me/971500999324" className="rounded-lg bg-[#7B8D6A] px-6 py-3.5 text-sm font-semibold text-white hover:opacity-95 transition-all shadow-sm flex items-center gap-1.5">
                WhatsApp us
              </a>
            </div>
          ...
          <div className="relative">
            <div className="p-3 border border-[#7B8D6A]/30 rounded-2xl">
            ...
            {treatments.map((t, idx) => (
              <div key={t.name} className="grid grid-cols-1 lg:grid-cols-[1fr_1.05fr] gap-14 items-center py-16 border-t border-border/60 first:border-0 first:pt-2">
                ...
                <div className={`relative ${idx % 2 === 1 ? 'lg:order-2' : ''}`}>
                  <div className="p-3.5 border border-[#7B8D6A]/40 rounded-2xl relative">
                    ...
                    <div className="absolute -right-3 top-8 bg-card border border-border/60 px-4 py-2.5 rounded-lg shadow-md z-10 flex flex-col items-start min-w-[100px]">
                      ...
                      <b className="font-display text-base text-[#7B8D6A] mt-0.5">{t.price.replace('From ', '')}</b>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col">
                  <span className="text-xs font-semibold tracking-widest text-[#7B8D6A] uppercase mb-3">
                    Treatment 0{idx + 1}
                  </span>
                    ...
                    <ul className="space-y-2 mb-6">
                      {t.points.map((p) => (
                        <li key={p} className="relative pl-6 text-sm text-foreground/90">
                          <span className="absolute left-0 text-[#7B8D6A] font-bold">✓</span>
                          {p}
                        </li>
                      ))}
                    </ul>
                  ...
                  <div className="flex gap-3 flex-wrap">
                    <a href="https://wa.me/971500999324" className="rounded-lg bg-[#7B8D6A] text-black px-5 py-2.5 text-xs font-semibold hover:opacity-95 transition-all shadow-sm">
                      Book {t.name}
                    </a>
                    <a href="https://wa.me/971500999324" className="rounded-lg bg-[#7B8D6A] px-5 py-2.5 text-xs font-semibold text-white hover:opacity-95 transition-all shadow-sm">
                      WhatsApp
                    </a>
                  </div>
                </div>
      ...
      <section className="py-20 bg-forest text-ivory">
        <div className="mx-auto max-w-6xl px-6 lg:px-10 grid gap-12 lg:grid-cols-2 items-center">
          <div>
            <p className="eyebrow mb-4 text-[#7B8D6A]">Why Al Nemah</p>
            ...
            <div className="flex flex-col gap-6">
              <div className="border-t border-ivory/20 pt-5 flex gap-4">
                <span className="font-display text-xl text-[#7B8D6A]">01</span>
                <div>
              ...
              <div className="border-t border-ivory/20 pt-5 flex gap-4">
                <span className="font-display text-xl text-[#7B8D6A]">02</span>
                <div>
              ...
              <div className="border-t border-ivory/20 pt-5 border-b border-ivory/20 pb-5 flex gap-4">
                <span className="font-display text-xl text-[#7B8D6A]">03</span>
                <div>
      ...
      <section className="py-20 bg-card border-t border-border/60">
        <div className="mx-auto max-w-6xl px-6 lg:px-10">
          ...
          <div className="grid gap-6 sm:grid-cols-3">
            {related.map((r) => (
              <Link key={r.slug} to={r.slug} className="block bg-background border border-border/60 hover:border-[#7B8D6A]/60 p-6 rounded-xl transition-all duration-300 hover:-translate-y-0.5">
                <span className="text-[10px] uppercase tracking-wider text-[#7B8D6A] font-semibold mb-2 block">
                  Explore Related
                </span>
      ...
      <section className="bg-forest text-ivory py-16 text-center border-t border-ivory/15">
        <div className="mx-auto max-w-4xl px-6">
          <p className="eyebrow text-[#7B8D6A] mb-4">Ready when you are</p>
